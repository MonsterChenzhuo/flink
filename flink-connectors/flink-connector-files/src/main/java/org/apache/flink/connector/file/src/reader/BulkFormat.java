 ...
 *
 * ulkFormat每次读取和解码一批记录。批量格式的例子有ORC或Parquet等格式。 外部的'BulkFormat'类主要充当配置容器和读取器的工厂。
 * 实际的阅读由BulkFormat完成。Reader，它是在createReader(Configuration, FileSourceSplit)方法中创建的。
 * 如果在检查点流执行期间基于检查点创建了批量读取器，那么将在restoreader (Configuration, FileSourceSplit)方法中重新创建该读取器。
 ...
 * 这并不一定适用于所有格式，这就是为什么拆分是可选的。 检查点 批量读取器对每批读取的数据返回一个迭代器。迭代器产生记录和位置。
 * 随着记录的处理，该位置以原子方式存储在检查点状态中。这意味着它必须是在记录被处理后可以恢复读取的位置;因此，位置实际上指向当前记录之后的记录。
 * 返回此位置信息的最简单方法是不存储偏移量，只存储一个递增的记录计数，以便在恢复后跳过。
 * 根据上述约定，返回第一个记录时，记录跳过计数为1，返回第二个记录时，记录跳过计数为2，以此类推。
 * 能够有效查找记录(或查找每个第n个记录)的格式可以使用位置字段直接查找记录，从而避免在恢复时读取和丢弃许多记录。
 * 注意:为什么我们不让位置指向当前记录，并且在恢复后总是跳过一条记录(刚刚处理的记录)?我们需要能够支持跳过记录(甚至一个)不是一个选项的格式。
 * 例如，执行(下推)过滤器的格式可能希望避免全部跳过记录计数，以便当过滤器围绕检查点/保存点进行更新时，它们不会跳过错误的记录。 可序列化的 与Flink中的许多其他API类一样，外部类是可序列化的，以支持将实例发送给分布式工作者以实现并行执行。这纯粹是RPC的短期序列化，没有一个实例会以序列化形式长期持久化。 批处理记录 在文件源内部，读取器将读取线程(执行通常阻塞的I/O操作)的批记录传递给执行流和批数据处理的异步邮箱线程。批量传递记录(而不是一次传递一个记录)大大减少了线程到线程的切换开销。 对于BulkFormat，一个批处理(由BulkFormat. reader . readbatch()返回)作为一个交付。
 */
@PublicEvolving
